...
from Net.model import fusion_net
...
# 載入預訓練模型
pre_model = torch.load('./weight/fusion_1127_adamw.pth')
pretrained_dict = pre_model.state_dict()
model = fusion_net(features=train_x.shape[1]).to(device)
...
torch.save(model, './weight/fusion_1127_sgd.pth')
...
# 預測結果
model = torch.load('./weight/fusion_1127_sgd.pth')
with torch.no_grad():
    ...
    my_submission.to_csv('./csv/fusion_1127_sgd.csv', index=False)
    print('寫入預測結果')