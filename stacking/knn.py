...
for train_index, test_index in kf.split(train_x):
    ...
    fold += 1
    knn = KNeighborsClassifier(n_neighbors=2, algorithm='auto', p=1)
    knn.fit(trainfold_x, trainfold_y)

    y_pred_test = knn.predict(test)
    y_pred_val = knn.predict(valfold_x)
...
my_submission.to_csv('{}.csv'.format('./csv/knn_fold_test'), index=False)
...
pd.DataFrame(dic2).to_csv('./csv/knn_fold_val.csv', index=False)